...
class ClassListSerializationTest extends AbstractIofXmlTestCase
{
    ...
    {
        $original = $this->deserializer->deserializeClassList(file_get_contents(__DIR__.'/fixtures/ClassList.xml'));
        $xml = $this->serializer->serializeClassList($original);

        $this->assertValidIofXml($xml);

        $reparsed = $this->deserializer->deserializeClassList($xml);
    ...
    {
        $original = $this->deserializer->deserializeClassList(file_get_contents(__DIR__.'/fixtures/ClassList_Individual_Step1.xml'));
        $xml = $this->serializer->serializeClassList($original);
    ...
    {
        $original = $this->deserializer->deserializeClassList(file_get_contents(__DIR__.'/fixtures/ClassList_Relay_Step1.xml'));
        $xml = $this->serializer->serializeClassList($original);
